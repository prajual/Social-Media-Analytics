...
def print_verbatims(df,keyword):
    ...
    for i,item in verbatims.head().iterrows():
        doc.append(item['message'])
    return doc

def facebook2(access,pop_in_post,pop_in_comment):
	import requests 
	import matplotlib.pyplot as plt
	import json 
	import pandas as pd
	...
	post=print_verbatims(df_posts,pop_in_post)
	comment=print_verbatims(df_comments,pop_in_comment)	
	return (post,comment)
